...
const ImageWithFallback: React.FC<{ src: string; alt: string; className?: string }> = ({ src, alt, className }) => {
  ...
  if (error) {
    return (
      <div className={`flex flex-col items-center justify-center bg-slate-50 text-slate-300 gap-2 ${className}`}>
        <ImageOff size={24} className="opacity-20" />
        <span className="text-[8px] font-black uppercase tracking-widest opacity-40">Asset Missing</span>
      </div>
    );
  }
...
const VisualArchive: React.FC<VisualArchiveProps> = ({ items, onAddItem, onDeleteItem }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {items.map((item) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-end">
               <p className="text-white text-[10px] font-black uppercase tracking-widest truncate">{item.title || 'Untitled'}</p>
            </div>
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Append Media</span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 animate-in fade-in duration-300 backdrop-blur-xl">
          ...
          <div className="w-full max-w-5xl px-8 flex flex-col items-center">
            ...
            <div className="mt-8 text-center space-y-2 max-w-2xl">
               <h4 className="text-white text-2xl font-black tracking-tight">{viewerItem.title || 'Archive Entry'}</h4>
               <p className="text-slate-400 text-sm font-medium leading-relaxed">{viewerItem.description || 'Captured documentation for management logs.'}</p>
               <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest pt-4">Captured: {new Date(viewerItem.createdAtUtc).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      )}
