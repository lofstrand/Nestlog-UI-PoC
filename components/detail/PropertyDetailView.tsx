...
import { MapPin, Zap, Users, Camera, AlertTriangle, X, Shield, ExternalLink } from 'lucide-react';
import { Property, Tag, Document, GalleryItem, HouseholdMember, Contact } from "../../types";
import DetailLayout from "../layout/DetailLayout";
import NotesSection from "../sections/NotesSection";
import TagsSection from "../sections/TagsSection";
import AttachmentsSection from "../sections/AttachmentsSection";
import PropertyTeamSection from "../sections/PropertyTeamSection";
...
const PropertyDetailView: React.FC<PropertyDetailViewProps> = ({ 
  ...
  const getEnergyColor = (rating?: string | null) => {
    switch (rating) {
      ...
      case 'B': return 'text-green-500 bg-green-50 border-green-100';
      case 'C': return 'text-lime-500 bg-lime-50 border-lime-100';
      case 'D': return 'text-yellow-500 bg-yellow-50 border-yellow-100';
      case 'E': return 'text-orange-500 bg-orange-50 border-orange-100';
  ...
  const handleAddMedia = (data: { type: 'image' | 'video', url: string, title: string, description: string }) => {
    ...
    onUpdateEntity('property', entity.id, { gallery: [...(entity.gallery || []), newItem] });
  };
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 space-y-10 shadow-sm">
             <VisualArchive 
               ...
               onDeleteItem={(id) => onUpdateEntity('property', entity.id, { gallery: (entity.gallery || []).filter(i => i.id !== id) })}
             />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 space-y-6 shadow-sm">
                ...
                <div className="flex items-center space-x-6">
                   <div className={`w-20 h-24 rounded-2xl border-2 flex flex-col items-center justify-center shadow-lg ${getEnergyColor(entity.energyRating)}`}>
                      <span className="text-[10px] font-black uppercase opacity-60">Rating</span>
                      <span className="text-4xl font-black">{entity.energyRating || '?'}</span>
                   </div>
                   <div className="space-y-3 flex-1">
                      <div>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Primary Heating</p>
                         <p className="text-sm font-bold text-slate-900">{entity.heatingSystemType || 'Not Logged'}</p>
                      </div>
             ...
             <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 space-y-6 shadow-sm">
                ...
                <div className="space-y-4">
                   <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Occupancy Classification</p>
                      <div className="flex items-center justify-between">
                         <p className="text-lg font-black text-slate-900 tracking-tight">{entity.occupancyStatus.replace(/([A-Z])/g, ' $1').trim()}</p>
                         <Badge color="text-[#5a6b5d]" bgColor="bg-white">Active Status</Badge>
                      </div>
                   </div>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="px-4 py-3 bg-white border border-slate-100 rounded-xl">
                         <p className="text-[9px] font-black text-slate-400 uppercase">Roof Structure</p>
                         <p className="text-xs font-bold text-slate-800 truncate">{entity.roofType || 'Unknown'}</p>
                      </div>
                      <div className="px-4 py-3 bg-white border border-slate-100 rounded-xl">
                         <p className="text-[9px] font-black text-slate-400 uppercase">Foundation</p>
                         <p className="text-xs font-bold text-slate-800 truncate">{entity.foundationType || 'Standard'}</p>
                      </div>
                   </div>
                </div>
             </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Geographic Address</p>
                    <p className="text-slate-900 font-bold text-2xl leading-tight tracking-tighter">
                      {entity.address.line1}<br />
                      <span className="text-slate-500 font-medium text-lg">{entity.address.city}, {entity.address.stateOrRegion} {entity.address.postalCode}</span>
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-5 bg-[#f2f4f2] rounded-[1.5rem] border border-[#e1e6e1] flex flex-col justify-center">
                      <p className="text-[9px] font-black text-[#5a6b5d] uppercase tracking-widest mb-1">Floor Area</p>
                      <div className="flex items-baseline space-x-1">
                        <p className="text-slate-900 font-black text-2xl tracking-tight">{entity.floorArea || '--'}</p>
                        <span className="text-[10px] font-bold text-[#5a6b5d]">m²</span>
                      </div>
                    </div>
                    <div className="p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 flex flex-col justify-center">
                      ...
                      <p className="text-slate-900 font-black text-2xl tracking-tight">{entity.constructionYear || '19XX'}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="relative bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-none">Delete Property?</h2>
                <p className="text-sm text-slate-500 font-medium leading-relaxed">
                  You are about to delete <strong>{entity.name}</strong>. This will permanently remove all associated maintenance logs, room registries, and document links.
                </p>
              </div>
