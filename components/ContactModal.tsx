...
import { X, User, Briefcase, Mail, Phone, StickyNote, Globe, Star, Siren, DollarSign, Hammer, Award, HardHat, ShieldAlert } from 'lucide-react';
...
const ContactModal: React.FC<ContactModalProps> = ({ isOpen, onClose, onSave, initialData, availableTags }) => {
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Household Registry</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-10">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <Input autoFocus label="Given Name" value={firstName} onChange={(e) => setFirstName(e.target.value)} required />
          ...
          <div className="pt-4 border-t border-slate-100 space-y-4">
             <div className="flex items-center justify-between p-6 bg-slate-900 rounded-[1.5rem] shadow-xl relative overflow-hidden group">
                ...
                <div className="flex items-start space-x-3 relative z-10">
                  <div className={`p-2 rounded-xl shadow-sm ${isEmergencyContact ? 'bg-[#b45c43] text-white animate-pulse' : 'bg-slate-800 text-slate-600'}`}>
                    <Siren size={16} />
                  </div>
                  <div>
                    <p className="text-sm font-black text-white leading-none">Emergency Priority</p>
                    <p className="text-[10px] text-slate-500 font-bold mt-1.5 uppercase tracking-widest">Pin to rapid dispatch dashboard.</p>
                  </div>
                </div>
                <button 
                  type="button"
                  onClick={() => setIsEmergencyContact(!isEmergencyContact)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none z-10 ${isEmergencyContact ? 'bg-[#b45c43]' : 'bg-slate-700'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isEmergencyContact ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
             </div>
          </div>
          ...
          <div className="space-y-6 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Operating Entity" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="e.g. Acme Plumbing Ltd." />
              <Input label="Professional Role" value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="e.g. Master Electrician" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="flex items-center justify-center space-x-2">
                         {[...Array(5)].map((_, i) => (
                           <button type="button" key={i} onClick={() => setRating(i + 1)}>
                             <Star size={24} className={i < rating ? "text-amber-400 fill-amber-400" : "text-slate-200"} />
                           </button>
                         ))}
                      </div>
                      <p className="text-center text-[10px] font-black text-slate-400 uppercase tracking-widest">Score: {rating}.0 / 5.0</p>
                   </div>
                ) : (
                  <button 
                    type="button" 
              ...
              <Input label="Hourly Engagement Rate" icon={DollarSign} type="number" value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value)} placeholder="0.00" />
            </div>
          </div>
          ...
          <div className="space-y-6 pt-4 border-t border-slate-100">
            ...
            <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-[1.5rem] flex items-start space-x-4">
               ...
               <div className="space-y-2">
                  <p className="text-sm font-black text-indigo-900 tracking-tight leading-none">Compliance Management</p>
                  <p className="text-xs text-indigo-700 leading-relaxed font-medium">Capture license numbers and technical domains to automate vendor verification during project planning.</p>
               </div>
            </div>
          </div>
          ...
          <div className="space-y-6 pt-4 border-t border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input label="Registry Email" icon={Mail} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@domain.com" />
              <Input label="Direct Line" icon={Phone} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 000 000 0000" />
            </div>
            <Input label="Digital Domain / Portal" icon={Globe} value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} placeholder="https://..." />
          </div>
